...
function showError(inputId: string, message: string): void {
    const inputElement = document.getElementById(inputId) as HTMLInputElement | HTMLSelectElement | null;
    if (!inputElement) { console.warn(`showError: Element with ID '${inputId}' not found.`); return; }
    const formGroup = inputElement.closest('.form-group');
    const errorSpan = formGroup?.querySelector('.error-message') as HTMLSpanElement | null;
    ...
    } else { console.warn(`showError: No .error-message span found near #${inputId}`); }
}

function clearError(inputId: string): void {
    const inputElement = document.getElementById(inputId) as HTMLInputElement | HTMLSelectElement | null;
    if (!inputElement) { console.warn(`clearError: Element with ID '${inputId}' not found.`); return; }
    const formGroup = inputElement.closest('.form-group');
    const errorSpan = formGroup?.querySelector('.error-message') as HTMLSpanElement | null;
...
// --- DOM Elements ---
const addDocForm = document.getElementById('addDocForm') as HTMLFormElement | null;
const addModuleSelect = document.getElementById('addModule') as HTMLSelectElement | null;
const addFileInput = document.getElementById('addFile') as HTMLInputElement | null;
const documentsTableBody = document.getElementById('documentsTableBody') as HTMLTableSectionElement | null;
const logoutBtn = document.getElementById('logoutBtnManage') as HTMLButtonElement | null;
...
function createTableRow(docId: string, docName: string, docModule: string): HTMLTableRowElement | null {
    try {
        ...
        availableModules.forEach(mod => {
             const option = document.createElement('option'); option.value = mod; option.textContent = mod; if (mod === docModule) option.selected = true; moduleEditSelect.appendChild(option);
         });
...
function loadDocuments() {
    ...
    if (!documentsTableBody) { console.error("Table body 'documentsTableBody' not found!"); return; }
    const simulatedDocs = [
    ...
    documentsTableBody.innerHTML = '';
     if (simulatedDocs.length === 0) {
        const tr = documentsTableBody.insertRow(); const td = tr.insertCell(); td.textContent = "Aucun document trouvé."; td.colSpan = 3; td.className = 'px-6 py-4 text-center text-sm text-gray-500';
     } else {
        simulatedDocs.forEach(doc => {
            const row = createTableRow(doc.id, doc.name, doc.module);
            if (row) { documentsTableBody.appendChild(row); }
        });
     }
...
if (addDocForm && addModuleSelect && addFileInput && documentsTableBody) {
    addDocForm.addEventListener('submit', (e: SubmitEvent) => {
        ...
        // Validation
        let formValid = true;
        if (!module) { showError("addModule", "Veuillez choisir un module."); formValid = false; }
        if (!file) { // Check if file is null
             showError("addFile", "Veuillez choisir un fichier PDF."); formValid = false;
        }
        else if (file.type !== "application/pdf") { showError("addFile", "Le fichier doit être au format PDF."); formValid = false; }

        if (!formValid) {
             console.warn("Add form validation failed.");
             const firstInvalid = addDocForm.querySelector('.invalid') as HTMLElement | null; firstInvalid?.focus();
        ...
        try {
            ...
            if (newRow) {
                 const noDocsRow = documentsTableBody.querySelector('td[colspan="3"]')?.closest('tr');
     ...
     if(!addDocForm) console.error("addDocForm is missing");
     if(!addModuleSelect) console.error("addModuleSelect is missing");
     if(!addFileInput) console.error("addFileInput is missing");
     if(!documentsTableBody) console.error("documentsTableBody is missing");
}

// --- Table Actions Event Delegation (Reviewed and Robust) ---
if (documentsTableBody) {
    documentsTableBody.addEventListener('click', (e: MouseEvent) => {
        const target = e.target as HTMLElement;
        const button = target.closest('button.table-button');
        if (!button) return;

        const row = button.closest('tr');
        if (!row) { console.warn("Could not find parent row for clicked button."); return; }

        // Get elements robustly using querySelector within the specific row
        const filenameDisplay = row.querySelector('.filename-display') as HTMLSpanElement | null;
        const filenameEditInput = row.querySelector('.filename-edit-input') as HTMLInputElement | null;
        const moduleDisplay = row.querySelector('.module-display') as HTMLSpanElement | null;
        const moduleEditSelect = row.querySelector('.module-edit') as HTMLSelectElement | null;
        const editBtn = row.querySelector('.edit-btn') as HTMLButtonElement | null;
        const saveBtn = row.querySelector('.save-btn') as HTMLButtonElement | null;
        ...
        if (button.classList.contains('edit-btn')) {
             ...
             filenameEditInput.classList.remove('hidden'); filenameEditInput.value = '';
             moduleDisplay.classList.add('hidden');
             moduleEditSelect.classList.remove('hidden'); moduleEditSelect.value = currentModule;
        ...
        // --- Cancel Button Click ---
            ...
            filenameEditInput.classList.add('hidden'); filenameEditInput.value = '';
            ...
            editBtn.classList.remove('hidden'); deleteBtn.classList.remove('hidden');
        }
        // --- Save Button Click ---
            ...
            if (newFile) {
                ...
                } else { console.log(` > File selected, name is the same.`); }
            } else { console.log(` > No new file selected.`); }
            ...
            if(fileChanged || moduleChanged) console.log(` > Visual changes applied (temporary).`);
            else console.log(` > No visual changes detected.`);
...
} else { console.error("Table body 'documentsTableBody' not found for event delegation."); }

// --- Initial Load ---
if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded', loadDocuments); }
else { loadDocuments(); } // If already loaded, run directly
