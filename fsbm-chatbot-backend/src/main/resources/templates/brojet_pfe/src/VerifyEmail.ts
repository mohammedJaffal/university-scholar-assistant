...
const userEmailPlaceholder = document.getElementById('userEmailPlaceholder') as HTMLElement | null;
...
if (verifyForm && verificationCodeInput) {
    verifyForm.addEventListener('submit', (e: SubmitEvent) => {
        e.preventDefault();
        const code = verificationCodeInput.value.trim();
        clearError('verificationCode');
        let isValidCode = true;

        if (!code) { showError("verificationCode", "Veuillez entrer le code de vérification."); isValidCode = false; }
        else if (code.length !== 6 || !/^\d+$/.test(code)) { showError("verificationCode", "Le code doit être composé de 6 chiffres."); isValidCode = false; }

        if (!isValidCode) { verificationCodeInput.focus(); return; }
...
} else { console.error("Formulaire de vérification ou champ de code introuvable."); }
...
if (resendCodeBtn) {
     resendCodeBtn.addEventListener('click', () => {
        ...
        resendCodeBtn.disabled = true; resendCodeBtn.textContent = "Code renvoyé..."; resendCodeBtn.classList.add('opacity-50', 'cursor-not-allowed');
        setTimeout(() => {
            resendCodeBtn.disabled = false; resendCodeBtn.textContent = "Renvoyer le code"; resendCodeBtn.classList.remove('opacity-50', 'cursor-not-allowed');
